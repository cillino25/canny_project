...
namespace my_Space
{
					...
					int aperture_size, bool L2gradient, int custom)
	{
		...
		// TEST: COPY IMAGES INTO NON-CACHABLE memor
		/*
		int fd;
		...
		*/
		...
		Mat dx(_src.size(), CV_16SC(cn));
		//dx.data = (unsigned char *) dx_p;
		Mat dy(_src.size(), CV_16SC(cn));
		//dy.data = (unsigned char *) dy_p;
		

		Mat gradient(_src.size(), CV_16SC(cn));

		//Compute partial derivatives using Sobel operator/kernel
		gettimeofday(&start, NULL);
		// NON-Cacheable memory test
		...
		//Normal version
		my_Space::Sobel(_src, dx, CV_16S, 1, 0, aperture_size, 1, 0, BORDER_REPLICATE, custom);		//x component of the gradient
		my_Space::Sobel(_src, dy, CV_16S, 0, 1, aperture_size, 1, 0, BORDER_REPLICATE, custom);		//y component of the gradient
	  gettimeofday(&stop, NULL);
	  printf("SobelDx AND SobelDy wall time: %lf s\n\n", ((stop.tv_sec + stop.tv_usec*0.000001)-(start.tv_sec + start.tv_usec*0.000001))*PRESC);
		
		// NON-Cacheable memory version: results must be copied back to dx and dy mats
	  //Mat dx;
	  //dx_tmp.copyTo(dx);
	  //Mat dy;
	  //dy_tmp.copyTo(dy);
		...
		if (L2gradient)
		{
			low_thresh = std::min(32767, (int)low_thresh);		//max positive number on 16 bit = 2^15 - 1
			high_thresh = std::min(32767, (int)high_thresh);

			if (low_thresh > 0) low_thresh *= low_thresh;
			if (high_thresh > 0) high_thresh *= high_thresh;
		}
		int low = (int)cvFloor(low_thresh);
		int high = (int)cvFloor(high_thresh);
